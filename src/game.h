 ...
 * state to GAME_ON.
 * @param  renderer    A pointer to the SDL_Renderer to be used for rendering
 * the game.
 ...
 */
void draw_triangle(SDL_Renderer *renderer, int circle_x, int circle_y,
                   int circle_radius, int triangle_x, int triangle_y);
 ...
 */
void draw_angular_sectors(SDL_Renderer *renderer, int x, int y,
                          int inner_radius, int num_sectors);
 ...
 */
void draw_obstacle(SDL_Renderer *renderer, int circle_x, int circle_y,
                   ArcObstacle *obstacle);
 ...
 * @param circle_y   The y-coordinate of the circle's center.
 */
void spawn_obstacle(ArcObstacle *obstacles, int sector, int circle_x,
                    int circle_y);

/**
 * Updates the state of an ArcObstacle based on its speed.
 *
 * @param obstacle     A pointer to an ArcObstacle structure representing the
 * arc obstacle.
 ...
 */
void update_triangle_position(int circle_x, int circle_y, int circle_radius,
                              int triangle_size, float angle, int *triangle_x,
                              int *triangle_y);
 ...
 * @param circle_x     The x-coordinate of the center of the obstacle's bounding
 * circle.
 * @param circle_y     The y-coordinate of the center of the obstacle's bounding
 * circle.
 * @param obstacle     A pointer to an ArcObstacle structure representing the
 * arc obstacle.
 * @return             True if the triangle is colliding with the obstacle,
 * false otherwise.
 */
bool check_collision(int triangle_x, int triangle_y, int triangle_size,
                     int circle_x, int circle_y, ArcObstacle *obstacle);
 ...
 */
void render_timer(SDL_Renderer *renderer, Uint32 startTime,
                  SDL_Texture *fontTexture, int charWidth, int charHeight,
                  int charsPerRow, SDL_Color color);
 ...
 * over     * screen.
 * @param game_over_texture A pointer to the SDL_Texture containing the game
 * over    * image.
 */
